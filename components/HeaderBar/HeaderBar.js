...
const HeaderBar = ({
	...
	const [queryInInput, setQueryInInput] = useState(undefined);
	const [tempMobile, setTempMobile] = useState(false);
	const [showLoginMenu, setShowLoginMenu] = useState(false);
	let max_page = 40;
	let mobile_max_page = 10;

	const NewValueSearch = (value) => {
	...
	const HandleSubmit = async (e) => {
		...
		if (tempMobile) {
			await router.replace({
				...
				query: { q: queryInInput, page_size: mobile_max_page },
			});
		} else {
			await router.replace({
				pathname: '/listado/all/index',
				query: { q: queryInInput, page_size: max_page },
			});
		}
							...
							<form onSubmit={(e) => HandleSubmit(e)}>
								<div className='header-bar__form-container'>
									<input
										...
										onChange={(e) => NewValueSearch(e.target.value)}
